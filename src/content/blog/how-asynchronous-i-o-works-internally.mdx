...
Rust’s `async` is not some black-box magic.  
It’s a **3-layer system** that uses the operating system, language features, and a runtime to let you handle thousands of tasks without burning thousands of threads.  
...
We ask the OS:
> "Here’s a list of sockets. Please wake me up when any of them is ready."
...
2. Call `epoll_wait()` → runtime goes to sleep.  
3. OS wakes it up when an event happens.  
4. Runtime dispatches the right handler for that event.  

👉 That’s how a single thread can handle **10,000 connections** without collapsing.
...
So far, we’ve seen how the **Reactor** (OS-level event notification) and **Futures** (Rust state machines) fit into async programming.  
But there’s still one missing piece:  

👉 **Who keeps polling the futures until they’re done?**  
...
- If a student says **"Yes" (Ready)** → check the homework and move on.  
- If a student says **"Not yet" (Pending)** → the student raises their hand (Waker) when they’re ready.  
- If nobody is ready, the teacher takes a nap until someone calls them.

That’s basically async Rust.
...
- It polls futures until they’re done.
...
⚡ Without the Executor, futures would never make progress.  
...
loop {
    if let Some(task) = runnable_queue.pop() {
        ...
        // No tasks ready → wait for OS to signal
        os_reactor_wait();
    }
}
``` 
